...
Finally, we say that a `number` is something that starts with a digit and contains only digits or a single decimal point. In other words, `10.0` is a number but `.3` is not. `10` is also a number, as is `10.5`. But `10.2.3` is not a number.
...
def eval_expression(expression):
    ...
    if the first lexeme in the expression is a left paren:
        ...
        # no parens, so must just be three values
        left = eval_expression([the first element of expression])
        right = eval_expression([the last element of expression])
        operator = the middle element of expression
...
If you then executed `LOAD r0, r2`, the value of `r2` would be set to 5.6. You can think of `STORE a, b` as `memory[a] = b`, and the line `LOAD a, b` as `b = memory[a]`.
